...
class CalculateRouteView(APIView):
    ...
    def post(self, request):
        ...
        if not pickup_coords and not pickup:
            return Response(
                {"error": "Pickup coordinates or address are required."},
                status=status.HTTP_400_BAD_REQUEST
            )
        if not drop_coords and not drop:
            return Response(
                {"error": "Drop coordinates or address are required."},
                status=status.HTTP_400_BAD_REQUEST
            )

        try:
            route_data = get_route(
                pickup or "Pickup Location",
                drop or "Drop Location",
            ...
            print("Calculate route error:", error)
            return Response(
